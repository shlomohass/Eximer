...
bool Exam::setCropExamArea(imagecontainer& image) {
	...
	float max_width = bounds.width,
		  max_height = bounds.height;

	//Create and fill the dst vector:
	image.exam.dst_base_area_points.push_back(cv::Point2f(bounds.x, bounds.y));
	image.exam.dst_base_area_points.push_back(cv::Point2f(bounds.x + bounds.width, bounds.y));
	image.exam.dst_base_area_points.push_back(cv::Point2f(bounds.x + bounds.width, bounds.y + bounds.height));
	image.exam.dst_base_area_points.push_back(cv::Point2f(bounds.x, bounds.y + bounds.height));
...
bool Exam::parseExamImage(examContainer& exam) {
	...
	cv::threshold(exam.proc.blur, exam.proc.thres_blur, 110, 255, CV_THRESH_BINARY_INV);
	...
	for (int i = 0; i < exam.proc.contours.size(); i++)
	{
		...
		exam.proc.mc[i] = cv::Point2f(exam.proc.mu[i].m10 / exam.proc.mu[i].m00, exam.proc.mu[i].m01 / exam.proc.mu[i].m00);
...
int Exam::filterCheckAndBound(examContainer& exam) {
	...
	for (int i = 0; i < exam.checkboxes.size(); i++) {
		cv::putText(
			exam.proc.traces,
			std::to_string(i + 1),
			cv::Point(exam.checkboxes[i].center.x + 23, exam.checkboxes[i].center.y + 7),
...
int Exam::createQuestions(examContainer& exam) {
	...
	float dist		= 0;
	float range		= SHQR_CHECKBOX_RANGE_MINMAX_BETWEEN;
	float average	= 0;
	float count		= 0;
	...
	for (int i = 0; i < (int)exam.questions.size(); i++) {
		...
		inflated_box = this->padVecRec(box, 5);
		this->drawVectorToLines(inflated_box, exam.proc.traces, CV_RGB(0, 204, 0), 2);
